...
def quit_app():
	"""Quit the running boot selector.
	"""
	exit()

def load_app(command, cwd=None):
	"""Load an application/emulator.

	Args:
		command (str): Command to execute.
		cwd (str, optional): Working directory if applicable. Defaults to None.
	"""
	try:
...
frmWelcome.grid(row=1, column=0, columnspan=dsize)
...
# Additional buttons.
frmAdditionals = tk.Frame(master=frmMain)
frmAdditionals.grid(row=4, column=0, columnspan=dsize)
frmClose = tk.Frame(master=frmAdditionals)
frmClose.grid(row=0, column=0)
btnClose = tk.Button(master=frmClose, text='Close', command=quit_app, font=font)
btnClose.pack()
frmShutdown = tk.Frame(master=frmAdditionals)
frmShutdown.grid(row=0, column=1)
